...
target_img = torch.FloatTensor(target_img).cpu().permute(2, 0, 1).unsqueeze(0)
...
#         print(args.gpu,"번째 데이터 계산후")
        ...
        if i % args.saveInterval == 0:
            torch.save(model.state_dict(), '{0}/craft_{1}_{2}.pth'.format(args.store_sample, epoch, i))
        ...
        if dist.get_rank() == 0:
            ...
            if i % 10 == 0:
                ...
                with torch.no_grad():
                    ...
                    cv2.imwrite(eval_saveimg_target_dir+str(count).zfill(6)+'.jpg', char_label)
                    count = count+1
